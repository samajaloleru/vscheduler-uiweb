<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{'name':'clients-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          + New
        </router-link>
      </div>
      ...
      <table-display>
        <template slot="tableHead">
          <tr class="tl bg-black black f7">
            ...
            <td class="">
              <input type="text" v-model="search.filter.state" placeholder="Address" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
            </td>
            <td class="">
        ...
        <template slot="tableBody" v-if="recordList.length > 0">
          <tr class="stripe-dark" v-for="(client, index) in recordList" :key="index">
            ...
            <td class=" pa2 f7">{{client.State}}</td>
      ...
      <div v-if="recordList.length < 1">
        <p class="black f6 tc ma3">There are currently no clients record within the system</p>
      </div>
...
<script type="text/javascript">
  ...
  export default {
    data() {return{
      url: "/api/clients", recordList: [],
      search: {text: "", field: "Fullname", limit: 20, page:1, skip: 0, filter:{}},
    }},
