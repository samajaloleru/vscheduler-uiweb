<template>
	<section>
		<div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
			...
			<router-link :to="{'name':'documents-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
				+ New
			</router-link>
		</div>
		...
		<table-display>
			<template slot="tableHead">
				<tr class="tl bg-black black f7">
					...
					<td class="">
						<input type="text" v-model="search.filter.filename" placeholder="Filename" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
					</td>
					<td class="">
						<input type="text" v-model="search.filter.filetype" placeholder="Filetype" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
					</td>
					<td class="">
				...
				<tr class="tl bg-black white">
					...
					<td class="pa2">Filename</td>
					<td class="pa2">Filetype</td>
					<td class="pa2">Document Type</td>
					<td class="pa2">Status</td>
				</tr>
			</template>
			<template slot="tableBody" v-if="recordList.length > 0">
				<tr class="stripe-dark" v-for="(document, index) in recordList" :key="index">
					<td class="tc">
						<router-link data-glyph="eye" class="f6 oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'documents-view',params:{id:document.ID}}" ></router-link>
					</td>
					<td class=" pa2"> 
						<span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{document.Title}}
					</td>
					<td class=" pa2">{{document.Owner}}</td>
					<td class=" pa2">{{document.Filename}}</td>
					<td class=" pa2">{{document.Filetype}}</td>
					<td class=" pa2">{{document.Doctype}}</td>
					<td class=" pa2">{{document.Workflow}}</td>
				</tr>
			</template>
		</table-display>

		<div v-if="recordList.length < 1">
			<p class="black f6 tc ma3">There are currently no documents record within the system</p>
		</div>
