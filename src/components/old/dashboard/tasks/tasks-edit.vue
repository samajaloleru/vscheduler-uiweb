<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{name:'tasks-view',params:{id:record.ID}}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          Go Back
        </router-link>
      </div>
          ...
          <div class="ph2" v-if="tabProps.personalShow">
            <p class="w-100 cf  gray tracked"> PERSONAL Information</p>
              ...
              <div class="fl w-30-l pa1">
                <input type="file" class="dn" @change="uploadImageDisplay('Image')" ref="taskImage"/>
                <img :src="record.Image" @error="record.Image = taskIcon" class="w5 br-100 bg-near-white pa1" @click="uploadImage('taskImage')" />
              </div>
      ...
      save () {
        ...
        .then((response) => {
          ...
          if(response.data.Body !== null){
            setTimeout(function(){
              app.$router.push({name:"tasks-view",params:{id:response.data.Body}})
            },1000)
          } else {
            app.isSave = true;
          }
