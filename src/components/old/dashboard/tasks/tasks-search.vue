<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{'name':'tasks-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          + New
        </router-link>
      </div>
      ...
      <table-display>
        <template slot="tableHead">
          <tr class="tl bg-black black f7">
            ...
            <td class="">
              <input type="text" v-model="search.filter.Title" placeholder="Task" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
            </td>
            <td class="">
          ...
          <tr class="tl bg-black white">
            ...
            <td class="pa2 ">Task</td>
            <td class="pa2 ">From User</td>
            <td class="pa2 ">To User</td>
            <td class="pa2 ">Start Date</td>
            <td class="pa2 ">Stop Date</td>
          </tr>
        </template>
        <template slot="tableBody" v-if="recordList.length > 0">
          <tr class="stripe-dark" v-for="(task, index) in recordList" :key="index">
            <td class="">
              <router-link data-glyph="eye" class="f7 oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'tasks-view',params:{id:task.ID}}" >
              </router-link>
            </td>
            <td class=" pa2">
              <span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{task.Workflow}}
            </td>
            <td class=" pa2 f7">{{task.Title}}</td>
            <td class=" pa2 f7">{{task.From}}</td>
            <td class=" pa2 f7">{{task.To}}</td>
            <td class=" pa2 f7">{{dateTimeConvert(task.StartDate)}}</td>
            <td class=" pa2 f7">{{dateTimeConvert(task.StopDate)}}</td>
          </tr>
        </template>
      </table-display>
      ...
      <div v-if="recordList.length < 1">
        <p class="black f6 tc ma3">There are currently no tasks record within the system</p>
      </div>
