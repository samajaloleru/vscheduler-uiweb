<template>
		<section>
			<div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
				...
				<router-link :to="{'name':'domains-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
					+ New
				</router-link>
			</div>
			...
			<table-display>
				<template slot="tableHead">
					<tr class="tl bg-black black f7">
						...
						<td class="">
							<input type="text" v-model="search.filter.domain" placeholder="Domain" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
						</td>
						<td class="">
							<input type="text" v-model="search.filter.proxy" placeholder="Proxy" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
						</td>
                        <td class="">
					...
					<tr class="tl bg-black white">
						...
						<td class="pa2">Domain</td>
						<td class="pa2">Proxy</td>
						<td class="pa2">Owner</td>
					</tr>
				</template>
				<template slot="tableBody" v-if="recordList.length > 0">
					<tr class="stripe-dark" v-for="(domain, index) in recordList" :key="index">
						<td class="tc">
							<router-link data-glyph="eye" class="f6 oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'domains-view',params:{id:domain.ID}}" ></router-link>
						</td>
						<td class=" pa2"> 
							<span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{domain.Workflow}}
						</td>
						<td class=" pa2">{{domain.Title}}</td>
						<td class=" pa2">{{domain.Domain}}</td>
						<td class=" pa2">{{domain.Proxy}}</td>
						<td class=" pa2">{{domain.Owner}}</td>
			...
			<div v-if="recordList.length < 1">
				<p class="black f6 tc ma3">There are currently no domains record within the system</p>
			</div>
