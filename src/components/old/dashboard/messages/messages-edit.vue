<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{name:'messages-view',params:{id:record.ID}}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          Go Back
        </router-link>
      </div>
      ...
      save () {
        ...
        .then((response) => {
          ...
          if(response.data.Body !== null){
            setTimeout(function(){
              app.$router.push({name:"messages-view",params:{id:response.data.Body}})
            },1000)
          } else {
            app.isSave = true;
          }
