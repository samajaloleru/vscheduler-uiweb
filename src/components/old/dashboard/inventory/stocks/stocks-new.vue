		...
		<section>
			<div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
				<div class="fl bg-light-gray pa2 pb fw5 tracked ttu f7 black ">
					Inventory / Stock Inventory / New
				</div>
				<router-link :to="{name:'stocks-search'}" class="fl right-1 absolute ph2 pv1 br4 bg-dark-red grow-ns pointer f6 white tc no-underline">
					Go Back
				</router-link>
			</div>
			...
			<div class="pa2 bg-light-gray br4 cf">
				...
				<div class="w-100">
					...
					<div class="fl pa2 mt3 mt0-ns w-100 w-third-l">
						<label class="db fw4 lh-copy f6 black"> Stock Location</label>
...
<script type="text/javascript">
	...
	export default {
		...
		data() {return{
			url: "/api/stocks",
			record: {Code:"stock", Currency:"USD", Workflow:"draft", Docdate: new Date().toISOString().slice(0,10)},
			notifications:[],
			partnerList:[],
			ownerList:[],
			isSave:true,
			...
			save () {
				...
				.then((response) => {
					...
					if(response.data.Body !== null){
						app.record = {Code:"stock", Currency:"USD"}
						app.$router.push({name:"stocks-edit",params:{id:parseInt(response.data.Body)}})
					}
				}).catch((e) => {
					console.log(e)
				})
