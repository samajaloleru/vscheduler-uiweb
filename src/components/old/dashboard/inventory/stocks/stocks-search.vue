<template>
	<section>
	  ...
	  <table-display>
		...
		<template slot="tableBody" v-if="recordList.length > 0">
		  <tr class="stripe-dark" v-for="(stock, index) in recordList" :key="index">
			<td class="">
			  <router-link data-glyph="eye" class="f7 oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'stocks-view',params:{id:stock.ID}}" >
			  </router-link>
			</td>
			<td class=" pa2">
			  <span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{stock.Docno}}
			</td>
			<td class=" pa2 f7">{{stock.Docdate}}</td>
			<td class=" pa2 f7">{{stock.Workflow}}</td>
			<td class=" pa2 f7">{{stock.Title}}</td>
			<td class=" pa2 f7">{{stock.Createdbyuser}} <small>{{dateTimeConvert(stock.Createdate)}}</small> </td>
			<td class=" pa2 f7">{{stock.Updatedbyuser}} <small>{{dateTimeConvert(stock.Updatedate)}}</small> </td>
		  </tr>
		</template>
	  </table-display>
	  ...
	  <div v-if="recordList.length < 1">
		<p class="black f6 tc ma3">There are currently no stocks record within the system</p>
	  </div>
...
<script type="text/javascript">
  ...
  export default {
	data() {return{
	  ...
	  search: {text: "", field: "Title", limit: 20, page:1, skip: 0, filter:{Code:"stock"}},
	}},
