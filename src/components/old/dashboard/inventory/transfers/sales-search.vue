      ...
      <table-display>
        <template slot="tableHead">
          <tr class="tl bg-black black f7">
            ...
            <td class="">
              <input type="text" v-model="search.filter.address" placeholder="Address" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
            </td>
            <td class="">
        ...
        <template slot="tableBody" v-if="recordList.length > 0">
          <tr class="stripe-dark" v-for="(sale, index) in recordList" :key="index">
            ...
            <td class=" pa2">
              <span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{sale.Fullname}}
            </td>
            <td class=" pa2 f7">{{sale.Phone}}</td>
            <td class=" pa2 f7">{{sale.Email}}</td>
            <td class=" pa2 f7">{{sale.Address}}</td>
