			...
			<tr class="">
                ...
                <td class="black f6 tc" colspan="4"> 
					<span v-if="recordList.length < 1">
						There are currently no other lines record  within the system
					</span>
				</td>
				<td class="w2 pa1 f7 tc">
	...
	methods: {
		searchRecords() {
	  	...
	  	searchItem(index) {
			...
			const app = this;
			const search = {text: app.recordList[index].Item, field: "Title", filter:{Code:"other"}, limit: 20, skip: 0};
		...
		addItem(){
            const app = this
			var curLine = {ID:0, Code:"other", StockID:parseInt(app.refid), Checked:false, Lineno:app.recordList.length+1, Quantity:1, Item:"", ItemID:0, Amount:0, Totalexcltax:0, itemList:[]}
		...
		saveLines(){
			...
			HTTP.post(app.url, recordLines, {withCredentials: true}) .then((response) => {
				...
				if (response.data.Code == 200) {
					setTimeout(function(){ app.$router.push({name:"others-view",params:{id:parseInt(app.refid)}}) },1000)
				}
			}).catch((e) => { console.log(e) })
        },
        updateParentTotal(){
