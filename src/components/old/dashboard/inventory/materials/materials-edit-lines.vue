	  ...
	  <table-display class="fl w-100">
		<template slot="tableHead" class="fixed">
			<tr class="tl bg-black white w-100">
				...
				<td class="pa1 w3">No</td>
				<td class="pa1 w5">Item</td>
				<td class="pa1 w4">Color</td>
				<td class="pa1 w3">UOM</td>
				<td class="pa1 w3">Qty</td>
				<td class="pa1 w4">Cost</td>
				<td class="pa1 w4">Total</td>
			</tr>
		</template>
		</table-display>
		...
		<template slot="tableBody">
			<tr class="stripe-dark" v-for="(line, index) in recordList" :key="index">
                ...
                <td class="ph1 w5">
                    <div class="items-center flex-row">
                        ...
                        <small class="ph1 br1 bg-red hover-bg-dark-red pointer white tc " @click="line.itemList=[],line.Item='',line.ItemID=0,line.Color='', line.Uom='', line.Unit=0, line.Quantity=0, line.Amount=0,line.Totalexcltax=0">
                            CLEAR
                        </small>
                    </div>
                    <div class="relative">
                        <ul class="bg-white absolute w-100 pa0 br2 br--bottom mt0 list overflow-scroll" style="max-height:6em">
                            <li class="pa2 black bt b--near-white" @click="line.ItemID = parseInt(item.ID), line.Item=item.Title, line.Color=item.Color, line.Uom=item.Uom, line.Unit=item.Unit, line.itemList = []" v-for="(item, index) in line.itemList" :key="index">
                            <span class="f7">#{{index+1}}</span> {{item.Title}} <small class="i">{{item.Color}}</small> <small class="i">{{item.Unit}}{{item.Uom}}</small>
                            </li>
                        </ul>
                    </div>
				</td>
                ...
                <td class="ph1 w4 f7">
					<input class="pa1 ba b--silver br2 bg-white w-100" type="text" min="1" readonly="readonly" v-model="line.Color">
				</td>

                <td class="ph1 w3 f7">
					<input class="pa1 ba b--silver br2 bg-white w-100" type="text" min="1" readonly="readonly" v-model="line.Uom">
				</td>
			...
			<tr class="">
                ...
                <td class="black f6 tc" colspan="6"> 
					<span v-if="recordList.length < 1">
						There are currently no material lines record  within the system
					</span>
				</td>
				<td class="w2 pa1 f7 tc">
	...
	methods: {
		searchRecords() {
	  	...
	  	searchItem(index) {
			...
			const app = this;
			const search = {text: app.recordList[index].Item, field: "Title", filter:{Code:"material"}, limit: 20, skip: 0};
		...
		addItem(){
            const app = this
			var curLine = {ID:0, Code:"material", StockID:parseInt(app.refid), Checked:false, Lineno:app.recordList.length+1, Quantity:1, Item:"", ItemID:0, Amount:0, Totalexcltax:0, itemList:[]}
		...
		saveLines(){
			...
			HTTP.post(app.url, recordLines, {withCredentials: true}) .then((response) => {
				...
				if (response.data.Code == 200) {
					setTimeout(function(){ app.$router.push({name:"materials-view",params:{id:parseInt(app.refid)}}) },1000)
				}
			}).catch((e) => { console.log(e) })
        },
        updateParentTotal(){
