<template>
		<section>
			<div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
				...
				<router-link :to="{'name':'campaigns-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
					+ New
				</router-link>
			</div>
			...
			<table-display>
				<template slot="tableHead">
					<tr class="tl bg-black black f7">
						...
						<td class="">
							<input type="text" v-model="search.filter.link" placeholder="Link" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
						</td>
						<td class="">
					...
					<tr class="tl bg-black white">
						...
						<td class="pa2">Link</td>
						<td class="pa2">Owner</td>
						<td class="pa2">Start</td>
						<td class="pa2">Stop</td>
					</tr>
				</template>
				<template slot="tableBody" v-if="recordList.length > 0">
					<tr class="stripe-dark" v-for="(campaign, index) in recordList" :key="index">
						<td class="tc">
							<router-link data-glyph="eye" class="f6 oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'campaigns-view',params:{id:campaign.ID}}" ></router-link>
						</td>
						<td class=" pa2"> 
							<span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{campaign.Workflow}}
						</td>
						<td class=" pa2">{{campaign.Title}}</td>
						<td class=" pa2">{{campaign.Link}}</td>
						<td class=" pa2">{{campaign.Owner}}</td>
						<td class=" pa2">{{dateTimeConvert(campaign.StartDate)}}</td>
						<td class=" pa2">{{dateTimeConvert(campaign.StopDate)}}</td>
					</tr>
				</template>
			</table-display>

			<div v-if="recordList.length < 1">
				<p class="black f6 tc ma3">There are currently no campaigns record within the system</p>
			</div>
