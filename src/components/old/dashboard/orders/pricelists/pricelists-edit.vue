<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{name:'pricelists-view',params:{id:record.ID}}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          Go Back
        </router-link>
      </div>
      ...
      <div v-if="isFound" class="bg-light-gray br4 cf pa2">
			...
			<pricelistlines :refid="$route.params.id" ref="pricelistlines" />
...
import pricelistlines from "@/components/dashboard/orders/pricelists/pricelists-edit-lines"
...
export default {
    ...
    components: { notify, pricelistlines },
    created () {
		...
		save () {
			...
			this.notifications.push(response.data)
				...
				if(response.data.Body !== null){
					app.$refs.pricelistlines.saveLines();
				} 
			}).catch((e) => { console.log(e) })
		},
		searchOwner(){
