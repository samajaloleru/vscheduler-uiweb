<template>
	<section class="ph2">
	  <table-display class="fl w-100 overflow-scroll" style="max-height:50vh">
		...
		<template slot="tableBody" v-if="recordList.length > 0">
            <tr class="stripe-dark" v-for="(sale, index) in recordList" :key="index">
                <td class=" pa1">
                    <span class="f7">#</span>{{sale.Lineno}}
                </td>
                <td class=" pa2"> {{sale.Quantity}}</td>
                <td class=" pa2 f7">{{sale.Item}}</td>
                <td class=" pa2 f7">{{sale.Amount}}</td>
                <td class=" pa2 f7">{{sale.Totalexcltax}}</td>
            </tr>
		</template>
	  </table-display>
	  ...
	  <div v-if="recordList.length < 1">
		<p class="black f6 tc ">There are currently no sale lines record within the system</p>
	  </div>
