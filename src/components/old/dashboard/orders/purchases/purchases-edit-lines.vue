			...
			<tr class="">
                ...
                <td class="black f6 tc" colspan="4"> 
					<span v-if="recordList.length < 1">
						There are currently no purchase lines record  within the system
					</span>
				</td>
				<td class="w2 pa1 f7 tc">
...
<script type="text/javascript">
  ...
  export default {
	data() {return{
	  url: "/api/purchaselines", recordList: [], notifications:[], itemList:[], isChecked: false, isSave: false,
	  search: {text: "", field: "Title", limit: 100, page:1, skip: 0, filter:{}, RefField:"PurchaseID", RefID: parseInt(this.refid) },
	}},
	props:['refid'],
	...
	methods: {
		...
		addItem(){
			const app = this
			var curLine = {ID:0, PurchaseID:parseInt(app.refid), Checked:false, Lineno:app.recordList.length+1, Quantity:0, Item:"", ItemID:0, Amount:0, Totalexcltax:0, itemList:[]}
		...
		saveLines(){
			...
			HTTP.post(app.url, recordLines, {withCredentials: true}) .then((response) => {
				...
				if (response.data.Code == 200) {
					setTimeout(function(){ app.$router.push({name:"purchases-view",params:{id:parseInt(app.refid)}}) },1000)
				}
			}).catch((e) => { console.log(e) })
		}
	}
