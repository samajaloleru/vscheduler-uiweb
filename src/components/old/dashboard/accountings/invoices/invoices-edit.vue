<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{name:'invoices-view',params:{id:record.ID}}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          Go Back
        </router-link>
      </div>
      ...
      <div v-if="isFound" class="bg-light-gray br4 cf pa2">
			...
			<div class="w-100">
				...
				<div class="fl pa2 mt3 mt0-ns w-100 w-50-m w-25-l">
					<label class=" fw4 lh-copy f6 black"> Client</label>
			...
			<invoicelines :refid="$route.params.id" ref="invoicelines" />
...
import invoicelines from "@/components/dashboard/accountings/invoices/invoices-edit-lines"
...
export default {
    ...
    components: { notify, invoicelines },
    created () {
		...
		save () {
			...
			this.notifications.push(response.data)
				...
				if(response.data.Body !== null){
					app.$refs.invoicelines.saveLines();
				} 
			}).catch((e) => { console.log(e) })
		},
		searchOwner(){
