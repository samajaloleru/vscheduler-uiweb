<template>
    <section>
      <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
        ...
        <router-link :to="{name:'workorders-view',params:{id:record.ID}}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
          Go Back
        </router-link>
      </div>
      ...
      <div v-if="isFound" class="bg-light-gray br4 cf pa2">
			...
			<workorderlines :refid="$route.params.id" ref="workorderlines" />
...
import workorderlines from "@/components/dashboard/productions/workorders/workorders-edit-lines"
...
export default {
    ...
    components: { notify, workorderlines },
    created () {
		...
		save () {
			...
			this.notifications.push(response.data)
				...
				if(response.data.Body !== null){
					app.$refs.workorderlines.saveLines();
				} 
			}).catch((e) => { console.log(e) })
		},
		searchOwner(){
