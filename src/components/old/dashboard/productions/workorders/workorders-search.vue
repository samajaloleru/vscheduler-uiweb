<template>
	<section>
	  <div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
		...
		<router-link :to="{'name':'workorders-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
		  + New
		</router-link>
	  </div>
	  ...
	  <table-display>
		...
		<template slot="tableBody" v-if="recordList.length > 0">
		  <tr class="stripe-dark" v-for="(workorder, index) in recordList" :key="index">
			<td class="">
			  <router-link data-glyph="eye" class="f7 oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'workorders-view',params:{id:workorder.ID}}" >
			  </router-link>
			</td>
			<td class=" pa2">
			  <span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{workorder.Title}}
			</td>
			<td class=" pa2 f7">{{workorder.Docno}}</td>
			<td class=" pa2 f7">{{workorder.Docdate}}</td>
			<td class=" pa2 f7">{{workorder.Owner}}</td>
			<td class=" pa2 f7">{{workorder.Partner}}</td>
			<td class=" pa2 f7">{{workorder.Workflow}}</td>
		  </tr>
		</template>
	  </table-display>
	  ...
	  <div v-if="recordList.length < 1">
		<p class="black f6 tc ma3">There are currently no workorders record within the system</p>
	  </div>
