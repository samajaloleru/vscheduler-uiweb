			...
			<tr class="">
                ...
                <td class="black f6 tc" colspan="4"> 
					<span v-if="recordList.length < 1">
						There are currently no workorder lines record  within the system
					</span>
				</td>
				<td class="w2 pa1 f7 tc">
...
<script type="text/javascript">
  ...
  export default {
	data() {return{
	  url: "/api/workorderlines", recordList: [], notifications:[], itemList:[], isChecked: false, isSave: false,
	...
	methods: {
		...
		addItem(){
			const app = this
			var curLine = {ID:0, WorkorderID:parseInt(app.refid), Checked:false, Lineno:app.recordList.length+1, Quantity:0, Item:"", ItemID:0, Amount:0, Totalexcltax:0, itemList:[]}
		...
		saveLines(){
			...
			HTTP.post(app.url, recordLines, {withCredentials: true}) .then((response) => {
				...
				if (response.data.Code == 200) {
					setTimeout(function(){ app.$router.push({name:"workorders-view",params:{id:parseInt(app.refid)}}) },1000)
				}
			}).catch((e) => { console.log(e) })
		}
	}
