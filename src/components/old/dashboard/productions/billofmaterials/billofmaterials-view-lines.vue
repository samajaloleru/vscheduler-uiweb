<template>
	<section class="ph2">
	  <table-display class="fl w-100 overflow-scroll" style="max-height:50vh">
		...
		<template slot="tableBody" v-if="recordList.length > 0">
            <tr class="stripe-dark" v-for="(billofmaterial, index) in recordList" :key="index">
                <td class=" pa1">
                    <span class="f7">#</span>{{billofmaterial.Lineno}}
                </td>
                <td class=" pa2"> {{billofmaterial.Quantity}}</td>
                <td class=" pa2 f7">{{billofmaterial.Item}}</td>
                <td class=" pa2 f7">{{billofmaterial.Amount}}</td>
                <td class=" pa2 f7">{{billofmaterial.Totalexcltax}}</td>
            </tr>
		</template>
	  </table-display>
	  ...
	  <div v-if="recordList.length < 1">
		<p class="black f6 tc ">There are currently no billofmaterial lines record within the system</p>
	  </div>
...
<script type="text/javascript">
  ...
  export default {
	data() {return{
	  url: "/api/billofmateriallines", recordList: [],
